...
<template>
  <o-article-template title="サークルカット規格チェック">
    <m-article-window head="作業概要">
      <template #head>
        <ul>
          <li>サークルカットがつぶれていないか、半分切れてしまっていないかチェックする</li>
        </ul>
      </template>
    </m-article-window>
    <m-article-window head="作業">
      <template #head>
        <m-article-paragraph secondhead="作業手順">
          <template #secondhead>
            <ol>
              <li>サークルカットがつぶれていないか、半分切れてしまっていないかを目視で確認してください。</li>
            </ol>
          </template>
        </m-article-paragraph>
        <m-article-paragraph secondhead="企画カード一覧">
          <template #secondhead>
            <o-kikaku-card-list />
          </template>
        </m-article-paragraph>
    ...
    <m-article-window head="作業チェックリスト">
      <template #head>
        ...
        <a-link-button v-show="items.length === selectItems.length" link="/fix/cut-check" width="350px" class="mt-12"
